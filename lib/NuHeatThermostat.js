...
module.exports = class NuHeatThermostat {
    ...
    updateValues(newValues) {
        if (this.isOnline(newValues)) {
            ...
            // current temperature
            var currentTemperature = Number(this.toHBTemperature(newValues.currentTemperature));
            ...
            var setPointTemperature = Number(this.toHBTemperature(newValues.setPointTemp));
    ...
    isOnline(newValues) {
        ...
        if (typeof onlineValue === 'string') {
            const normalizedValue = onlineValue.replace(/'/g, '').trim().toLowerCase();
