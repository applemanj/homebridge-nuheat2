...
class NuHeatGroup {
    ...
    async setAwayMode(value, callback) {
        this.log.debug("Setting group away mode to " + value, this.deviceData.groupName);
        const response = await this.NuHeatAPI.setAwayMode(this.deviceData.groupId ?? 0, value);
        if (!response) {
