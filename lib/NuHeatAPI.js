...
class NuHeatAPI {
    ...
    async setAwayMode(groupId, awayMode) {
        const callURL = (0, settings_1.buildNuHeatApiUrl)("/api/v1/Group");
        const callOptions = {
            body: JSON.stringify({
                groupId,
                awayMode,
    ...
    async setHeatSetpoint(serialNumber, setPointTemp, holdLength) {
        ...
        const callURL = (0, settings_1.buildNuHeatApiUrl)("/api/v1/Thermostat");
        const callBody = {
            serialNumber,
            setPointTemp,
            scheduleMode,
    ...
    async refreshGroup(groupId) {
        const callURL = (0, settings_1.buildNuHeatApiUrl)("/api/v1/Group/" + groupId);
    ...
    async refreshThermostat(serialNumber) {
        const callURL = (0, settings_1.buildNuHeatApiUrl)("/api/v1/Thermostat/" + serialNumber);
        return await this.makeAPICall(callURL, {}, {
            normalize: NuHeatModels_1.normalizeThermostat,
        });
    }
    ...
    async getAccount() {
        const callURL = (0, settings_1.buildNuHeatApiUrl)("/api/v1/Account");
        return await this.makeAPICall(callURL, {}, { normalize: NuHeatModels_1.normalizeAccount });
    }
    async refreshSchedule(serialNumber) {
        const callURL = (0, settings_1.buildNuHeatApiUrl)("/api/v1/Schedule/" + serialNumber);
        return await this.makeAPICall(callURL, {}, {
            normalize: NuHeatModels_1.normalizeSchedule,
        });
    }
    async refreshSchedules() {
        const callURL = (0, settings_1.buildNuHeatApiUrl)("/api/v1/Schedule");
        return await this.makeAPICall(callURL, {}, {
            normalize: NuHeatModels_1.normalizeSchedule,
            normalizeArray: true,
        });
    ...
    async refreshEnergyLogDay(serialNumber, date) {
        const callURL = (0, settings_1.buildNuHeatApiUrl)("/api/v1/EnergyLog/Day/" + serialNumber + "/" + date);
        return await this.makeAPICall(callURL, {}, {
            normalize: NuHeatModels_1.normalizeEnergyUsage,
        });
    }
    async refreshEnergyLogWeek(serialNumber, date) {
        const callURL = (0, settings_1.buildNuHeatApiUrl)("/api/v1/EnergyLog/Week/" + serialNumber + "/" + date);
        return await this.makeAPICall(callURL, {}, {
            normalize: NuHeatModels_1.normalizeEnergyUsage,
        });
    }
    async refreshEnergyLogMonth(serialNumber, year) {
        const callURL = (0, settings_1.buildNuHeatApiUrl)("/api/v1/EnergyLog/Month/" + serialNumber + "/" + year);
        return await this.makeAPICall(callURL, {}, {
            normalize: NuHeatModels_1.normalizeEnergyUsage,
        });
    }
    ...
    findForm(html, selector) {
        return (html
            .querySelectorAll("form")
            .find((form) => form.querySelector(selector)) || html);
    }
    getFormAction(form, baseUrl, fallbackUrl) {
        return new URL(form.getAttribute?.("action") || fallbackUrl, baseUrl).toString();
    }
    getInputValue(html, name) {
        return (html.querySelector("input[name=" + name + "]")?.getAttribute("value") ||
            "");
    }
    getCheckedInputValues(html, name) {
        const values = [];
    ...
    getHtmlMetaRefreshUrl(htmlText, baseUrl) {
        const html = parse(htmlText);
        const metaRefresh = html
            .querySelectorAll("meta")
            .find((node) => (node.getAttribute("http-equiv") || "").toLowerCase() === "refresh");
    ...
    async followHtmlMetaRefresh(response, cookie) {
        if (response.headers.get("location")) {
            return response;
        }
        const redirectUrl = this.getHtmlMetaRefreshUrl(await response.text(), response.url);
        if (!redirectUrl) {
            return response;
        }
        this.log.debug("NuHeatAPI: Following OAuth browser redirect to " +
    ...
    getRedirectUrl(response) {
        ...
        return new URL(location, settings_1.NUHEAT_API_AUTHORIZE_URI);
    }
    isFinalAuthorizationRedirect(response) {
        const redirectUrl = this.getRedirectUrl(response);
        if (!redirectUrl) {
            return false;
        }
        return (redirectUrl.searchParams.has("code") ||
            redirectUrl.searchParams.has("error"));
    }
    needsOAuthConsent(response) {
        const redirectUrl = this.getRedirectUrl(response);
        if (!redirectUrl || this.isFinalAuthorizationRedirect(response)) {
            return false;
        }
        const redirectPath = redirectUrl.pathname.toLowerCase();
        return (redirectPath === "/connect/authorize/callback" ||
    ...
    async oauthLogin(authPage) {
        ...
        if (cookie) {
            ...
            const requestVerificationToken = this.getInputValue(loginForm, "__RequestVerificationToken");
            const requestReturnURL = this.getInputValue(loginForm, "ReturnUrl") ||
    ...
    async oauthConfirm(authPage, sessionCookie) {
        ...
        const callbackCookie = this.trimSetCookie(confirmPage.headers.raw?.()["set-cookie"]);
        ...
        if (confirmRedirectUrl?.pathname.toLowerCase() === "/consent") {
            ...
            if (!confirmPage) {
                this.log.error("NuHeatAPI: Unable to access the OAuth consent confirmation page.");
                return null;
            }
        }
        ...
        if (consentSessionCookie) {
            ...
            const loginPageHtml = parse(htmlText);
            const consentForm = this.findForm(loginPageHtml, "input[name=ScopesConsented]");
            const requestVerificationToken = this.getInputValue(consentForm, "__RequestVerificationToken");
            const requestReturnURL = this.getInputValue(consentForm, "ReturnUrl") ||
                this.getInputValue(loginPageHtml, "ReturnUrl");
            if (!requestVerificationToken) {
            ...
            const checkedScopes = this.getCheckedInputValues(consentForm, "ScopesConsented");
            ...
            this.log.debug("NuHeatAPI: OAuth consent required. Confirming scopes: " +
                this.getRequestedScope());
            const consentUrl = this.getFormAction(consentForm, confirmPage.url, settings_1.NUHEAT_API_CONSENT_URI);
            ...
            const responseCookie = this.trimSetCookie(response.headers.raw?.()["set-cookie"]);
            return await this.followHtmlMetaRefresh(response, this.mergeCookies(consentSessionCookie, responseCookie));
        }
        return null;
    }
    async oauthRedirect(loginResponse, sessionCookie) {
        ...
        const cookie = this.mergeCookies(sessionCookie, this.trimSetCookie(loginResponse.headers.raw?.()["set-cookie"]));
        if (cookie) {
    ...
    async getRefreshedAccessToken() {
        ...
        const refreshTokenRotated = !!previousRefreshToken && token.refresh_token !== previousRefreshToken;
