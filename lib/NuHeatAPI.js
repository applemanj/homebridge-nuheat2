...
module.exports = class NuHeatAPI {
    constructor(email, password, log, options = {}) {
        ...
        this.log = log;
        this.oauthClientId = options.clientId || process.env.NUHEAT_API_CLIENT_ID || NUHEAT_API_CLIENT_ID;
        this.oauthClientSecret = options.clientSecret || process.env.NUHEAT_API_CLIENT_SECRET || NUHEAT_API_CLIENT_SECRET;
        this.oauthRedirectUri = options.redirectUri || process.env.NUHEAT_API_REDIRECT_URI || NUHEAT_API_REDIRECT_URI;
        this.usingFallbackCredentials = !options.clientId && !process.env.NUHEAT_API_CLIENT_ID
    ...
    async fetch(url, options = {}, decodeResponse = true, isRetry = false) {
        ...
        try {
            ...
            if(!response.ok && !isRedirect(response.status)) {
                this.log.error("NuHeatAPI: " + url + " Error: " + response.status + " " + response.statusText);
                return null;
            }
            return response;
        } catch(error) {
